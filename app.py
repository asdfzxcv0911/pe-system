...
st.markdown("""<style>
    ...
    .student-row { 
        ...
        height: 36px; border: none !important;
    }
...
with tab2:
    ...
    if not df_score.empty:
        ...
        # 穩定性修復：確保 test_item 隨時有效
        if mode == "現有項目":
            test_item = st.selectbox("項目", ["體適能-800m", "體適能-仰臥捲腹", "體適能-立定跳遠", "體適能-坐姿體前彎", "平時成績"])
        else:
            test_item = st.text_input("輸入名稱", placeholder="請在此輸入測驗名稱")
        
        # 只有在有輸入名稱時才顯示名單，避免 Bug
